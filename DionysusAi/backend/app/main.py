...
ENABLE_RAG_BATCH = os.getenv("ENABLE_RAG_BATCH", "false").lower() == "true"
...
async def lifespan(app: FastAPI):
    ...
    model_loaded = qpa_agent.load_model(
        model_path=os.path.join(base_dir, "wine_rf_regressor.pkl"),
...
async def get_analysis():
    ...
    try:
        ...
        high_quality_count = sum(1 for s in samples if s.quality_score >= 7.0)  
        ...
        return {
            ...
            "high_quality_percentage": (high_quality_count / total_samples) * 100 if total_samples > 0 else 0,
...
async def get_pairing_recommendations(wine: WineFeatures):
    """Get food pairing recommendations based on chemical analysis"""
    try:    
        wine_dict = wine.model_dump()
