...
class LLMEnhancedRecommender:
    def __init__(self, llm_agent=None, dataset_path: str = None):
        self.dataset_path = dataset_path or r"C:\Users\User\Desktop\Python Code\kaggle_data\WineDataset.csv"
        self.llm_agent = llm_agent  # Your OllamaWineMentor (preferred if available)
    ...
    def recommend_with_explanation(
        ...
        results = self.collection.query(
            ...
            n_results=limit + 1,  # get one extra in case of filtering issues
...
Be warm, engaging, and concise (around 2 paragraphs max)."""
        ...
        for meta, doc in zip(results['metadatas'][0][:limit], wines_found[:limit]):
            recommendations.append({
                ...
                "full_context": doc  # for debugging
            })
